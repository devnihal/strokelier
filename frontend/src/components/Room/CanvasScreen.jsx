...
export default function CanvasScreen({ roomState, myPlayer, socket, roleInfo: parentRoleInfo }) {
  ...
  const currentStrokeDistance = useRef(0);
  const lastPoint = useRef(null);
  const boundingBox = useRef({ minX: 1, maxX: 0, minY: 1, maxY: 0 });
  ...
  const handlePointerDown = (e) => {
    ...
    currentStrokeDistance.current = 0;
    lastPoint.current = pt;
    boundingBox.current = { minX: pt.x, maxX: pt.x, minY: pt.y, maxY: pt.y };
    
    // Record first cell (10x10 grid)
    const cellX = Math.floor(pt.x * 10);
    const cellY = Math.floor(pt.y * 10);
  ...
  const handlePointerMove = (e) => {
    ...
    // 1. Grid Coverage Tracking
    const cellX = Math.max(0, Math.min(9, Math.floor(pt.x * 10)));
    ...
    boundingBox.current.maxX = Math.max(boundingBox.current.maxX, pt.x);
    boundingBox.current.minY = Math.min(boundingBox.current.minY, pt.y);
    boundingBox.current.maxY = Math.max(boundingBox.current.maxY, pt.y);

    // Vandalism Scoring System
    let vandalismScore = 0;
    
    // Penalty for extreme distance
    if (currentStrokeDistance.current > 5.0) vandalismScore += 1;
    if (currentStrokeDistance.current > 8.0) vandalismScore += 1;
    
    // Penalty for dense canvas coverage
    if (touchedCells.current.size > 40) vandalismScore += 1;
    if (touchedCells.current.size > 60) vandalismScore += 1;
    
    // Penalty for drawing across the entire board
    const boxArea = (boundingBox.current.maxX - boundingBox.current.minX) * (boundingBox.current.maxY - boundingBox.current.minY);
    if (boxArea > 0.6) vandalismScore += 1;
    if (boxArea > 0.85) vandalismScore += 1;
      ...
      <div className="canvas-layout">
        ...
        {showScribbleWarning && (
          <div className="modal-scrim open">
            <div className="shape-citadel play theme-role" style={{ border: 'none' }}>
              ...
              <h2 style={{ color: 'var(--wax-red)', marginTop: 0, fontFamily: 'var(--font-heading)' }}>VANDALISM DETECTED</h2>
              <p style={{ fontFamily: 'var(--font-serif)', color: 'var(--bone)', marginBottom: '8px' }}>
                Suspicious drawing behavior detected. Sabotaging the game by attempting to cover the canvas in a single stroke is not allowed.
              </p>
              <p style={{ fontFamily: 'var(--font-serif)', color: 'var(--wax-red)', marginBottom: '24px', fontWeight: 'bold' }}>
                Repeated offenses will result in getting kicked out of the room.
              </p>
              <Button variant="danger" onClick={closeWarning}>I Understand</Button>
            </div>
          </div>
        )}
    </div>
